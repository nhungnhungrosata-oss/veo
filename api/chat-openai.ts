...
for (let i = 1; i <= 10; i++) {
    ...
    if (key) {
        apiKeyManager.addKey('openai', { key, tier: 'free', label: `ENV OpenAI Key ${i}` });
    }
}
if (process.env.OPENAI_API_KEY) {
    apiKeyManager.addKey('openai', { key: process.env.OPENAI_API_KEY as string, tier: 'free', label: `ENV OpenAI Base Key` });
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const response = await aiAdapter.chat('openai', messages, options);
