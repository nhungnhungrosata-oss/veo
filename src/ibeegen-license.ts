...
// DOI APP_ID RIENG CHO TUNG WEB, vi du: veoday, veoday3d, hocvien, appvideo
const APP_ID = "veo";

const DEVICE_KEY_STORAGE_KEY = `ibeegen_device_key_${APP_ID}`;
const TRIAL_START_KEY = `ibeegen_trial_started_at_${APP_ID}`;

// Dung thu 3 ngay. Neu muon khoa ngay, doi thanh 0.
...
export function getTrialInfo(now = Date.now()): TrialInfo {
  ...
  return {
    active: TRIAL_DURATION_MS > 0 && remaining_ms > 0,
    started_at,
    expires_at,
    remaining_ms,
